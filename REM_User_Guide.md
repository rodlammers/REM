...
-   [Brief Introduction to REM](#brief-introduction-to-rem)
-   [Model Inputs](#model-inputs)
    -   [Sediment transport options](#sediment-transport-options)
    -   [Bank erosion](#bank-erosion)
    -   [Basic model inputs](#basic-model-inputs)
    -   ["Reach" Input](#reach-input)
    -   ["Profile" Input](#profile-input)
    -   [Channel geometry](#channel-geometry)
    -   [Bed grain size distribution](#bed-grain-size-distribution)
    -   [Discharge and sediment supply](#discharge-and-sediment-supply)
    -   [Bank soil properties](#bank-soil-properties)
    -   [Meandering](#meandering)
    -   [Knickpoint migration](#knickpoint-migration)
    -   [Model assumptions/fixed values](#model-assumptionsfixed-values)
-   [Running REM](#running-rem)
-   [Model Outputs and Post-processing](#model-outputs-and-post-processing)
-   [Monte Carlo Simulations](#monte-carlo-simulations)
-   [Editing code and recompiling](#editing-code-and-recompiling)
...
This user guide will provide only a brief discussion of model features. For more details on the model structure and code see Lammers and Bledsoe (2018b) or Lammers (2018).

Brief Introduction to REM
-------------------------

REM mechanistically simulates watershed-scale channel evolution, accounting for bed aggradation and incision as well as bank erosion, meander bend migration, and knickpoint migration. REM uses specific stream power (*ω*) to drive these calculations, where *ω* is a function of discharge, slope, and width. This eliminates the need for detailed hydraulic simulations to calculate flow depth and shear stress and reduces model complexity and uncertainty. Briefly, sediment transport rates are calculated throughout the channel network. Changes in channel bed elevation are computed based on a sediment mass balance --- the channel incises if transport capacity is greater than sediment supply and aggrades otherwise. REM can also simulate incision into cohesive bed material. Fluvial bank erosion is simulated using a simple excess shear stress approach (shear stress is calculated directly from *ω* using a new empirical relationship). Bank mass failure is simulated using a simplified version of the Bank Stability and Toe Erosion Model (BSTEM) (Simon et al. 2000, Simon, Pollen-Bankhead, and Thomas (2011)). Finally, REM can also simulate meander bend migration (e.g. increases in channel sinuosity) and knickpoint migration (see below for more detailed discussions). A user-specified proportion of the eroded bank, cohesive bed, and knickpoint soil are added to the bed material load (e.g. sand and coarser) while the remainder is assumed to be washload (e.g. silt and clay) and is transported to the watershed outlet with no in-channel or floodplain storage. REM can also simulate pollutant loading from bank and cohesive bed erosion. The user simply provides the soil concentration of said pollutant (e.g. phosphorus) and this pollutant loading is computed based on the mass of eroded soil. Finally, REM can also run Monte Carlo simulations, allowing the user to quantify uncertainty in model outputs based on uncertainty in model inputs.
...
These global inputs include cross section spacing (dx; m), time step for sediment transport/bed evolution modeling (dt; s), time step for user supplied discharge series (dt\_Q; s), type of sediment transport equation to use (type), bank erosion process to model (bank\_erosion), time step to print outputs (dt\_output; \# of dt\_Q time steps), whether or not to include meandering (meandering; 0 (N) or 1(Y)), the type of input (input\_type; "reach" or "profile"), dimensionless critical specific stream power for sediment mobilization (omegac\_star; 0.1 is a good default), exponent for sediment transport hiding function (b; -0.8 is a good default), number of Monte Carlo simulations (MC), and maximum number of CPU threads to use for these simulations (max\_threads). These options will be discussed in more detail below.

### Sediment transport options

REM can simulate sediment transport rates using four different sediment transport equations. One bedload and one total load equation were developed specifically for REM (Lammers and Bledsoe 2018a) based on Bagnold (1980):

*q*<sub>*b*</sub> = 1.43 × 10<sup>−4</sup>(*ω*−*ω*<sub>*c*</sub>)<sup>3/2</sup>*D*<sub>*s*</sub><sup>−1/2</sup>*q*<sup>−1/2</sup>
*Q*<sub>*t*</sub> = 0.0214(*ω*−*ω*<sub>*c*</sub>)<sup>3/2</sup>*D*<sub>*s*</sub><sup>−1</sup>*q*<sup>−5/6</sup>
 Where *q*<sub>*b*</sub> is the bedload transport rate (kg m<sup>-1</sup> s<sup>-1</sup>), *Q*<sub>*t*</sub> is the total load concentration (ppm), *ω* and *ω*<sub>*c*</sub> are specific and critical specific stream power, respectively (W m<sup>-2</sup>), *D*<sub>*s*</sub> is grain size (m), and *q* is unit discharge (m<sup>2</sup> s<sup>-1</sup>). REM can either simulate bedload transport (type = bedload), total load transport (type = total), or both (type = both). If "both" is selected, the bedload equation will be used for all grain sizes &gt;4 mm and the total load equation for all finer grain sizes.

In addition to these options, two other bedload transport equations are built into REM. The first is a purely empirical relationship fit to a large bedload transport database (Parker, Clifford, and Thorne 2011):

$$q\_{b\*} = \\begin{cases}
    100 \\omega\_\*^6, & \\text{for}\\ \\omega\_\* &lt; 0.25 \\\\
    0.2 \\omega\_\* ^ {1.5}, & \\text{for}\\ \\omega\_\* \\geq 0.25
    \\end{cases}$$

Where *q*<sub>*b*\*</sub> and *ω*<sub>\*</sub> are dimensionless bedload transport rate and specific stream power, respectively. To use this bedload equation, specify type = Parker.

The final sediment transport option is from Eaton and Church (2011):

$$E^\* = \\left\[0.92 - 0.25 \\left\[\\frac{\\omega\_{c\*}}{\\omega\_\*}\\right\] ^ {1/2}\\right\] ^ 9$$
...
$$\\frac{\\omega\_{ci\*}}{\\omega\_{c50\*}} = \\left(\\frac{D\_i}{D\_{50}}\\right)^{b} $$
 Where *ω*<sub>*c**i*\*</sub> and *ω*<sub>*c*50\*</sub> are the dimensionless critical specific stream power for the *i*<sup>*t**h*</sup> grain size and *D*<sub>50</sub>, respectively. A good approximation for *ω*<sub>*c*50\*</sub> is 0.1 (omegac\_star in "Model Inputs.txt") and for *b* is -0.8 (Lammers and Bledsoe 2018b).
...
Distance between each XS. Set individually for "profile" input but globally for "reach" input.
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Simulation time step for modeling bed evolution
</td>
<td style="text-align:left;">
NA
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Discharge time step \[s\]
</td>
<td style="text-align:left;width: 35em; ">
Time step of provided discharge data
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
Bedload or total load transport (or both depending on grain size)
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
None, mass failure, fluvial erosion, or both
</td>
<td style="text-align:left;">
NA
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Output time step (\# of discharge dt)
</td>
<td style="text-align:left;width: 35em; ">
Time step for printing model outputs (e.g. a value of 5 would print outputs every 5 discharge time steps)
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
Switch to include meandering (1) or not (0)
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
"Profile" or "reach" - see text for more details
</td>
<td style="text-align:left;">
NA
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Hiding function coef. (*ω*<sub>*c*</sub>)
</td>
<td style="text-align:left;width: 35em; ">
Single value, defaults to 0.1
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Hiding function exponent (b)
</td>
<td style="text-align:left;width: 35em; ">
Single value, defaults to 0.8
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Maximum number of CPU threads to use for Monte Carlo simulations (0 = use all available)
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
Discharge by reach at specified time step (e.g. daily, hourly, 15-min)
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Sediment transport rate for each reach at same time step as discharge. Can also be set to a fraction of the transport capacity.
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Bed elevation by cross section ("profile" input) or by reach ("reach" input).
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
One set of grain sizes but unique bed surface fractions by reach
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Unique by reach for left and right banks (total height, including toe)
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Unique by reach for left and right banks (less than bank height)
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Unique by reach for left and right banks (max 90 deg.)
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Unique by reach for left and right floodplain
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Floodplain angle \[degrees\]
</td>
<td style="text-align:left;width: 35em; ">
Unique value per reach
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Mannings n value for the main channel, unique value by reach
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Floodplain roughness (n)
</td>
<td style="text-align:left;width: 35em; ">
Mannings n value for the floodplain, unique value by reach
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Matrix describing connection of reaches in network
</td>
<td style="text-align:left;">
NA
</td>
...
<td style="text-align:left;width: 35em; ">
Critical shear stress of the bank soil
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Bank erodibility \[m**<sup>3</sup> N**<sup>−1</sup> s**<sup>−1</sup>\]
</td>
<td style="text-align:left;width: 35em; ">
Erodibility of bank soil (calculated by REM if not supplied)
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Cohesion of bank soil (can be different for bank and toe)
</td>
<td style="text-align:left;">
Y
</td>
...
<td style="text-align:left;width: 35em; ">
Friction angle of bank soil (can be different for bank and toe)
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Bank soil weight \[kN m**<sup>−3</sup>\]
</td>
<td style="text-align:left;width: 35em; ">
Saturated weight of bank soil (can be different for bank and toe)
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Bank/bed bedload proportion
</td>
<td style="text-align:left;width: 35em; ">
Proportion of the eroded bank or cohesive bed material (unique to each) to be added to the bank material load (sand and coarser)
</td>
<td style="text-align:left;">
Y
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Soil pollutant conc \[mg kg **<sup>−1</sup>\]
</td>
<td style="text-align:left;width: 35em; ">
Pollutant conc. (e.g. phosphorus) in the eroded bank/bed material
</td>
<td style="text-align:left;">
Y
</td>
...
<tr grouplength="2">
<td colspan="3" style="border-bottom: 1px solid;">
<strong>Cohesive Bed Erosion</strong>
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Depth to cohesive layer \[m\]
</td>
<td style="text-align:left;width: 35em; ">
Depth below initial bed elevation to cohesive material for each XS.
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Critical shear stress of cohesive bed material
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Average bend radius of curvature by reach
</td>
<td style="text-align:left;">
Y
</td>
...
<tr grouplength="5">
<td colspan="3" style="border-bottom: 1px solid;">
<strong>Knickpoint Migration</strong>
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
Reach in which the knickpoint is initially
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Initial location downstream from beginning of reach
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Elevation of the top of the knickpoint
</td>
<td style="text-align:left;">
N
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
K**<sub>*d*</sub> \[cm Pa**<sup>−1</sup>\]
</td>
<td style="text-align:left;width: 35em; ">
Knickpoint soil erodibility
</td>
<td style="text-align:left;">
N
</td>
...
<td style="text-align:left;width: 35em; ">
Main control file for global inputs (see text for details)
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
List of inputs to be varied in Monte Carlo simulations
</td>
<td style="text-align:left;">
Only for MC
</td>
...
<td style="text-align:left;width: 35em; ">
List of outputs to be printed for each Monte Carlo simulation
</td>
<td style="text-align:left;">
Only for MC
</td>
...
<td style="text-align:left;width: 35em; ">
Matrix describing how reaches are spatially organized (two rows, one column per reach)
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Initial channel bed elevations - one row per reach for "reach" input or one column per reach ("profile" input)
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Cross section spacing - one column per reach for "profile" input
</td>
<td style="text-align:left;">
Only for "profile" input
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
Length of reaches (1 value per reach)
</td>
<td style="text-align:left;">
Only for "reach" input
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
One row per reach (reach input) or two columns per reach (profile input) - Cohesive bed *τ*<sub>*c*</sub> and depth below initial bed elevation
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Initial channel bottom width (1 row per reach)
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Floodplain geometry (1 row per reach) - FP angle, left FP width, right FP width
</td>
<td style="text-align:left;">
Defaults to zero
</td>
...
<td style="text-align:left;width: 35em; ">
Initial left bank geometry (1 row per reach) - total bank height, toe height, bank angle, toe angle
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Initial right bank geometry (see "Input LB geometry.txt")
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Mannings roughness coefficients (1 row per reach) - channel n, fp n
</td>
<td style="text-align:left;">
Yes
</td>
...
<tr grouplength="2">
<td colspan="3" style="border-bottom: 1px solid;">
<strong>Grain Size Files</strong>
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
Grain sizes of bed grain size distribution
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Initial grain size distribution (1 row per reach) - proportion of each grain size class (Input Ds.txt) initially on channel bed (must sum to one for each reach)
</td>
<td style="text-align:left;">
Yes
</td>
...
<tr grouplength="2">
<td colspan="3" style="border-bottom: 1px solid;">
<strong>Discharge and Sediment Supply Files</strong>
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
Flow series (one column per reach) - each row is values at a single timestep (e.g. daily, 15-min)
</td>
<td style="text-align:left;">
Yes
</td>
...
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
Input sed supply.txt
</td>
<td style="text-align:left;width: 35em; ">
Sediment supply series (one column per reach) - each row is values at single discharge timestep
</td>
<td style="text-align:left;">
Defaults to zero
</td>
...
<tr grouplength="1">
<td colspan="3" style="border-bottom: 1px solid;">
<strong>Bank Properties File</strong>
</td>
</tr>
<tr>
<td style="text-align:left; padding-left: 2em;" indentlevel="1">
...
<td style="text-align:left;width: 35em; ">
Bank soil properties (1 row per reach) - *τ*<sub>*c*</sub>, Erodibility, Bank cohesion, Toe cohesion, Bank *ϕ*′, Toe *ϕ*′, Bank weight, Toe weight, Bank contaminant concentration, Bed contaminant concentration, Bank bedload proportion, Bed bedload proportion
</td>
<td style="text-align:left;">
Yes
</td>
...
<td style="text-align:left;width: 35em; ">
Initial meander geometry (1 row per reach) - sinuosity and radius of curvature
</td>
<td style="text-align:left;">
No
</td>
...
<td style="text-align:left;width: 35em; ">
Initial knickpoint locations and properties (1 row per knickpoint) - reach with knickpoint, initial distance downstream, initial elevation of top, height, erodibility
</td>
<td style="text-align:left;">
No
</td>
...
<p class="caption">
Drawing of hypothetical watershed (left) and how the channel network is represented in REM (right). The upstream end of each reach is numbered.
</p>
...
The only other input file that is different between the "reach" and "profile" input type is the cohesive bed input file ("Input bed cohesive.txt" file). For the "reach" input type, there are two columns: the first is the critical shear stress (*τ*<sub>*c*</sub>) of the cohesive bed material (in Pa) and the second is the depth below the initial bed elevation to the cohesive layer (in m). There is one row per reach. For our example, we don't want to include a cohesive bed so we will set it very far (50 m) below the channel bed (if the "Input bed cohesive.txt" file is not included, the bed *τ*<sub>*c*</sub> and depth to cohesive layer default to zero).
...
<p class="caption">
Schematic of modeled cross section geometry. Based on the user-supplied inputs, a geometry consisting of 10-points (gray circles) is constructed.
</p>
...
In this example, any discharge above 0.0158 m<sup>3</sup> s<sup>-1</sup> has some small sediment supply in the headwater channels only. Note that these are total sediment transport rates which are multiplied by the *initial*, user-supplied grain size distribution to get sediment transport by grain size.
...
The first column is the critical shear stress (*τ*<sub>*c*</sub>; Pa) of the bank toe soil. The second column is soil erodiblity (*k*; m<sup>3</sup> N<sup>-1</sup> s<sup>-1</sup>). If this value is set to 0 (as it is here) REM calculates soil erodiblity based on *τ*<sub>*c*</sub> (Simon, Thomas, and Klimetz 2010):

*k* = 1.6 × 10<sup>−6</sup>*τ*<sub>*c*</sub><sup>−0.826</sup>
 The next six columns are used in the bank stability modeling. First, are two columns of soil cohesion for the bank and bank toe, respectively (kPa). Next are the friction angle for bank and bank toe (degrees). Finally, there is the saturated unit weight of the bank and bank toe (kN m<sup>-3</sup>). For our examples, these values are the same for the bank and toe soil layers. The next two columns are the pollutant concentration of the bank (e.g. 350) and cohesive bed and knickpoint soil (e.g. 0; both in mg pollutant/kg soil). The final two columns are the fraction of the bank and cohesive bed/knickpoint soil that are bed material load (e.g. sand and coarser). The remainder of this eroded soil is washload (silt and clay).

### Meandering

REM simulates meander migration and subsequent channel elongation. This provides an additional mode by which evolving channels can reduce their slope (longer channels = higher sinuosity = lower slope). To do this, REM calculates the higher bank shear stresses observed on the outside of meander bends as a function of bend tightness (bend radius of curvature divided by channel width) (Army Corps of Engineers 1970):

$$\\tau\_{max} = 2.65 \\tau\_w \\left( \\frac{R\_c}{w} \\right)$$
 where *τ*<sub>*m**a**x*</sub> is the maximum bank shear stress on the outside of the bend (Pa), *τ*<sub>*w*</sub> is the wall/bank shear stress in the straight channel (Pa), *R*<sub>*c*</sub> is the radius of curvature (m), and *w* is channel width (m).

REM assumes the meandering channel is made up of circular arc segments. It does not simulate complex bend geometry, down-valley meander translation, or meander cutoff. To initially build the sinuous channel, and subsequently track changes in channel sinuosity, the user must supply an initial sinuosity and average *R*<sub>*c*</sub> value for each reach. For our example, we will assume the channel gets more sinuous moving downstream. We will also increase the *R*<sub>*c*</sub> value but keep the ratio of *R*<sub>*c*</sub> to channel width consistent among reaches.
...
This "Input meandering.txt" file has two columns: the first specifies initial sinuosity for each reach and the second is initial *R*<sub>*c*</sub> for each reach. To include meandering in the model, the user must set the `meandering` value to 1 in the "Model Inputs.txt" control file.

### Knickpoint migration

The final module that REM incorporates is a knickpoint migration algorithm (Allen et al. 2018). This algorithm calculates the rate of knickpoint reatreat as a function of cumulative daily discharge, knickpoint height, and knickpoint soil erodibility. Knickpoint migration is highly sensitive to the selected value of soil erodiblity and we suggest that this value be calibrated to give realistic migration rates (see Lammers (2018) or Lammers and Bledsoe (2018b) for more information). To model knickpoint migration, the user must input the reach the knickpoint is located in, its distance downstream from the top of that reach, its elevation, its height, and its erodibility (see figure below).
...
<p class="caption">
Schematic showing example of knickpoint in model. The user must specify the knickpoints location from the upstream end of the reach, its elevation, and its height. Note that the slope of the sub-reach is the same above and below the knickpoint.
</p>
...
This "Input knickpoint.txt" file has one row per knickpoint. The first column is the reach in which the knickpoint is located, the second is the distance downstream from the top of that reach (m), the third is the elevation of the top of the knickpoint (m), the fourth is the height of the knickpoint (m), and the final value is the knickpoint erodibility (cm Pa<sup>-1</sup>). As long as the "Input knickpoint.txt" file is included in the model folder, REM will simulate knickpoint migration. To turn off this module, simply delete the "Input knickpoint.txt" file.
...
-   Water specific weight = 9,810 N m<sup>-3</sup>
-   Soil bulk density (for eroded washload) = 1,600 kg m<sup>-3</sup>
...
<p class="caption">
Dialogue box shown during model run.
</p>
...
<p class="caption">
Output summary file created for each model run.
</p>
...
<tr>
<td style="text-align:left;">
Model Output Summary.txt
</td>
<td style="text-align:left;width: 40em; ">
Summary of model settings used, model run time, and any warning messages
</td>
</tr>
<tr>
<td style="text-align:left;">
Output bank loading.txt
</td>
<td style="text-align:left;width: 40em; ">
Fine sediment ("Sed") and pollutant ("P") loading (kg) for each reach at each discharge time step. Includes loading from bank erosion and cohesive bed and knickpoint erosion (if applicable).
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Cumulative mass of bed material load exported from the watershed (kg) output at discharge time step
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Bed sediment *D*<sub>50</sub> for each cross section.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
XS spacing for each cross section. Only changes if "meandering" = 1.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output knick locations.txt
</td>
<td style="text-align:left;width: 40em; ">
Locations of any knickpoints (distance downstream from each XS, in meters).
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Meander bend radius of curvature for each cross section.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output sediment vol.txt
</td>
<td style="text-align:left;width: 40em; ">
Cumulative volume (m**<sup>3</sup>) of sediment exported from the watershed from various sources for each discharge time step. See text for more details on each.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output sinuosity.txt
</td>
<td style="text-align:left;width: 40em; ">
Sinuosity of each reach.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Bed slope for each cross section.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output stream power.txt
</td>
<td style="text-align:left;width: 40em; ">
Specific stream power (W m**<sup>−2</sup>) for each cross section.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Average of channel bottom and top width (m) for each cross section.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output XS geometry all.txt
</td>
<td style="text-align:left;width: 40em; ">
Cross section geometry for all XS. Columns 2-11 are station (x) values and columns 12-21 are elevation (y) values (both in meters). For each time step, all XS are listed in order by reach moving downstream.
</td>
</tr>
<tr>
<td style="text-align:left;">
Output XS geometry.txt
</td>
<td style="text-align:left;width: 40em; ">
Cross section geometry for most upstream XS of each reach. Same format as "Output XS geometry all.txt"
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;width: 40em; ">
Bed elevation (m) for each cross section.
</td>
</tr>
</tbody>
</table>
Most outputs are printed at the user-specified time step ("dt\_output"). In these cases, the first column of the output is the discharge time step. Outputs for each reach are printed on a separate line (for all cross sections within the reach). For example, we can look at the first several outputs for modeled average channel width (average of bottom and top width).
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-32-1.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-32-2.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-32-3.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-32-4.png" style="display: block; margin: auto;" /> This shows initial (black) and final (red) longitudinal profiles for all modeled reaches.
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-33-1.png" style="display: block; margin: auto;" />
...
![](C:/Users/rlammers/Documents/WORK/Network%20Model%20v2/Documentation/Test%20Case/Figs/dz_out.gif)
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-36-1.png" style="display: block; margin: auto;" />

We see incision and widening in the "urbanized" part of the watershed (top left), with deposition of mobilized sediment at tributary junctions. Althought not observed here, the channel can actually show net narrowing if the banks collapse, deposit a failed soil block at the base of the bank, and this is not eroded by the flow. It may also be interesting to see how bed *D*<sub>50</sub> varied over the course of the simulation.

``` r
D50_plot(path = path)
```

<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-37-1.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-37-2.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-37-3.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-37-4.png" style="display: block; margin: auto;" />

This plots changes in *D*<sub>50</sub> (mm) over time for all cross sections in each reach (purple being more upstream and yellow being more downstream). Generally, there is coarsening at all cross sections. If finer sediment is supplied from upstream or from local bank erosion, the *D*<sub>50</sub> drops.
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-38-1.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-38-2.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-38-3.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-38-4.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-39-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-40-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-41-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-43-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-45-1.png" style="display: block; margin: auto;" /> Or, you might be interested in how stream power varies spatially throughout the network over time:

``` r
plot_omega(path, type = 1)
```

<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-46-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-47-1.png" style="display: block; margin: auto;" />
<table>
<thead>
<tr>
<th style="text-align:left;">
...
The `sinuosity_plot()` function also returns the change in sinuosity for each reach over the course of the simulation. This simulation showed a really significant (and possibly unrealistic) increase in channel sinuosity. Our estimate of bank *τ*<sub>*c*</sub> may have been to low, causing excessive meander migration. Additionally, REM caps sinuosity at 2.5 to provide a control on this type of excessive meander migration.

``` r
Rc_lines(path_meandering)
```

<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-48-1.png" style="display: block; margin: auto;" /><img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-48-2.png" style="display: block; margin: auto;" />

In the headwater reaches, *R*<sub>*c*</sub> actually decreases first before increasing. This is because REM assumes channel bends are circular arc segments:
...
If the angle of the bend is &lt;180°, meander migration causes tighter bends (i.e. reduced *R*<sub>*c*</sub>). As the bend angle increases above 180°, meander migration increases *R*<sub>*c*</sub>.
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-50-1.png" style="display: block; margin: auto;" />
...
<table class="table table-condensed" style="width: auto !important; margin-left: auto; margin-right: auto;">
<caption>
R functions for displaying REM output.
</caption>
<thead>
<tr>
<th style="text-align:left;">
...
<td style="text-align:left;">
Returns average widths for each reach, plots if specified
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots changes in bed *D*<sub>50</sub> for each XS within each reach
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots changes in bed elevation for the upstream XS of each reach (type = 1) or all XS in each reach (type = 2)
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Creates three network plots showing uncertainty in modeled changes in bed elevation
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Network plot of changes in bed elevation. Can also create a gif of simulation over time.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Creates a grain size distribution given a series of grain sizes, *D*<sub>50</sub>, and spread of the distribution
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Shows position over time of any knickpoints in the reaches. Also prints the annual migration rate.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Creates three series of plots: changes in bed elevation, change in width, and change in width-depth ratio throughout the channel network over time.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots specific stream power longitudinally by reach (type = 1) or over time by reach (type = 2).
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots either cumulative or daily fine sediment and pollutant loading for the watershed
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots channel longitudinal profiles (type = 1 all reaches on same plot; type = 2 each reach has own plot)
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots meander bend radius of curvature over time for all XS by reach
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Network plot of sediment loading by reach. Either for single model run (n\_MC = 0) or MC results (n\_MC &gt; 0).
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots watershed sediment inflow and outflow by day.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots sinuosity over time by reach. Also returns values for sinuosity change over the course of the simulation.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots channel bed slope over time for each cross section by reach.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots width-depth ratio for each cross section by reach. If "plot = FALSE", returns the computed values.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots changes in average channel width over time for each cross section by reach.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Creates three network plots showing uncertainty in modeled changes in channel width
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Network plot of changes in channel width. Can also create a gif of simulation over time.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots initial and final XS geometry for the upstream most XS in each reach.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots initial and final XS geometry for any specified XS in the channel network.
</td>
</tr>
<tr>
<td style="text-align:left;">
...
<td style="text-align:left;">
Plots XS geometry over time for any specified XS in the channel network.
</td>
</tr>
</tbody>
</table>
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-54-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-55-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-56-1.png" style="display: block; margin: auto;" />
...
<img src="REM_User_Guide_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-57-1.png" style="display: block; margin: auto;" />
...
2.  Make the following changes to the "Project Properties" in Visual Studio:
    -   Add the file path to the downloaded boost library under "C/C++\\General\\Additional Include Directories" (e.g. "C:\\boost\_1\_63\_0\\")
    -   Add the boost lib file path under "Linker\\General\\Additional Library Directories" (e.g. "C:\\boost\_1\_63\_0\\stage\\lib")
    -   Enable code parallelization by changing "C/C++\\Code GenerationLibrary" to "Multi-threaded (/MT)" and "C/C++\\Language\\Open MP Support" to "Yes (/openmp)"
...
Allen, Peter M., Jeffrey G. Arnold, Lance Auguste, Joseph White, and John Dunbar. 2018. “Application of a simple headcut advance model for gullies.” *Earth Surface Processes and Landforms* 43: 202–17. doi:[10.1002/esp.4233](https://doi.org/10.1002/esp.4233).

Army Corps of Engineers. 1970. “Hydraulic design of flood control channels.” EM 1110-2-1601, Department of the Army.

Bagnold, R A. 1980. “An empirical correlation of bedload transport rates in flumes and natural rivers.” *Proceedings of the Royal Society of London: Series A, Mathematical and Physical Sciences* 372 (1751): 453–73.

Eaton, B C, and M Church. 2011. “A rational sediment transport scaling relation based on dimensionless stream power.” *Earth Surface Processes and Landforms* 36: 901–10. doi:[10.1002/esp.2120](https://doi.org/10.1002/esp.2120).

Lammers, R W. 2018. “Modeling stream evolution and its consequences for watershed scale pollutant loading.” PhD Dissertation, Colorado State University.

Lammers, R W, and B P Bledsoe. 2018a. “Parsimonious sediment transport equations based on Bagnold’s stream power approach for river modeling and management.” *Earth Surface Processes and Landforms* 43: 242–58. doi:[10.1002/esp.4237](https://doi.org/10.1002/esp.4237).

Lammers, R W, and Brian P. Bledsoe. 2018b. “A network scale, intermediate complexity model for simulating channel evolution over years to decades.” *Earth ArXiv*. doi:[10.31223/osf.io/zgekr](https://doi.org/10.31223/osf.io/zgekr).

Parker, C, N J Clifford, and C R Thorne. 2011. “Understanding the influence of slope on the threshold of coarse grain motion: Revisiting critical stream power.” *Geomorphology* 126 (1-2). Elsevier B.V.: 51–65. doi:[10.1016/j.geomorph.2010.10.027](https://doi.org/10.1016/j.geomorph.2010.10.027).

Simon, A, A Curini, S E Darby, and E J Langendoen. 2000. “Bank and near-bank processes in an incised channel.” *Geomorphology* 35 (3-4): 193–217. doi:[10.1016/S0169-555X(00)00036-2](https://doi.org/10.1016/S0169-555X(00)00036-2).

Simon, A, N Pollen-Bankhead, and R E Thomas. 2011. “Development and application of a deterministic bank stability and toe erosion model for stream restoration.” In *Stream Restoration in Dynamic Fluvial Systems: Scientific Approaches, Analyses, and Tools*, edited by A Simon, SJ Bennett, and JM Castro, 453–74. Washington, D.C.: American Geophysical Union.

Simon, A, R E Thomas, and L Klimetz. 2010. “Comparison and experiences with field techniques to measure critical shear stress and erodibility of cohesive deposits.” *2nd Joint Federal Interagency Conference, Las Vegas* 826.
